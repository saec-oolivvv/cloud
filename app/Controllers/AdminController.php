...
try {
    ...
    public function assignStorage(string $id): void
    {
        ...
        $tenantRootPath = '/cloud/tenant_' . $id;
...
// Créer le dossier racine cloud/tenant_{id} + chemin complet sur le storage distant (isolation totale)
try {
    $adapter = AdapterFactory::fromDatabase($providerId);
    // 1. Créer le dossier racine cloud/
    $adapter->mkdir('/cloud');
    // 2. Créer le dossier tenant
    $adapter->mkdir($tenantRootPath);
    // 3. Créer l'arborescence complète dans le dossier tenant
    $adapter->mkdir($fullRemotePath);
    error_log("[STORAGE] Created cloud/ + tenant root + path: /cloud + {$tenantRootPath} + {$fullRemotePath}");
} catch (\Throwable $e) {
