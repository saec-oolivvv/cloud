# SAEC Sync - Tauri Desktop Client - Final Status Report
...
- **Rust**: 1.98.1 (stable)
- **Tauri**: 2.11.5
- **WebKitGTK**: 4.1 (webkit2gtk-4.1-devel installed)

## Blocker: Tauri Build Script Linker Failure

**Error**: `linking with cc failed: exit status: 1` in `tauri` (build script)

**Root Cause**: Tauri's build script compiles a test program that links against WebKitGTK. The test program fails to link due to missing libraries or incorrect linker flags in the build script environment.
...
- `pkgconf-pkg-config` ✓
- `openssl-devel` ✓
- Additional: `libpciaccess-devel`, `libdrm-devel`, `libunwind-devel`, `libgudev-devel`, `mesa-libgbm-devel`, `gstreamer1-plugins-base-devel`, `elfutils-devel`, `check-devel`
...
# libwebkit2gtk-4.1.so (libc6,x86-64) => /lib64/libwebkit2gtk-4.1.so
```
...
- Install `webkit2gtk-4.1` runtime: `sudo dnf install webkit2gtk-4.1`
- Try `export LIBRARY_PATH=/usr/lib64:/lib64` before build
- Try `export CC=clang` or different linker
- Try Tauri 1.x (different build system): `tauri = { version = "1", features = ["tray-icon"] }`
...
**Recommendation**: Try Tauri 1.x (which uses a different build approach) or investigate `webkit2gtk-4.1` runtime package installation for the build script's test linking.